...
class GradientRegressor():
    ...
    def fit(self, xs, ys, niter=100):
        ...
        for it in range(niter):
            ...
            dlda = torch.mean(2 * xs**2 * self.a + 2*self.b * xs - 2*xs*ys)
            dldb = torch.mean(2*xs  * self.a + 2*self.b - 2*ys)

            # Updated values
            self.a.data = self.a - self.lr * dlda
            self.b.data = self.b - self.lr * dldb
