...
class SailingGridworld(object):
    ...
    def _get_next_states_wind(self, state, action):
        ...
        transitions = [(desired_state, self.rewards[state[0], state[1]],
                        self.terminate[state[0], state[1]], 1-4*self.wind_p),
                       (desired_up, self.rewards[desired_up[0], desired_up[1]],
                        self.terminate[desired_up[0], desired_up[1]], self.wind_p),
                       (desired_down, self.rewards[desired_down[0], desired_down[1]],
                        self.terminate[desired_down[0], desired_down[1]], self.wind_p),
                       (desired_left, self.rewards[desired_left[0], desired_left[1]],
                        self.terminate[desired_left[0], desired_left[1]], self.wind_p),
                       (desired_right, self.rewards[desired_right[0], desired_right[1]],
    ...
    def _get_possible_transitions_standard(self, state, action):
        ...
        transitions.append((state1, reward1, terminate1, self.wrong_action_prob))
        ...
        transitions.append((state2, reward2, terminate2, self.wrong_action_prob))
        ...
        transitions.append((state3, reward3, terminate3, 1-2*self.wrong_action_prob))
        return transitions
