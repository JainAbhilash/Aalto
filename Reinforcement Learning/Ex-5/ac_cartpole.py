...
from ac_agent_task_4 import Agent, Policy,Value
...
def train(env_name, print_things=True, train_run_id=0, train_episodes=5000):
    ...
    for episode_number in range(train_episodes):
        ...
        while not done:
            ...
            #Task 2 add another parameter:
            action, action_probabilities,value = agent.get_action(observation,episode_number=episode_number)
            prev_observation = observation
            # Perform the action on the environment, get new state and reward
            observation, reward, done, info = env.step(action.detach().cpu().numpy())
            #observation_tensor_2 = torch.from_numpy(observation).float().to(train_device)
            # Store action's outcome (so that the agent can improve its policy)
            agent.store_outcome(prev_observation, action_probabilities, action, reward,value,done)
            ...
            #Task 4
            if timesteps % 10 == 0:
                _,_,value_new=agent.get_action(observation)
                agent.episode_finished(value_new,episode_number)
        ...
        #Task 3
        #agent.episode_finished(torch.tensor([0.], device=torch.device("cuda:0" if torch.cuda.is_available() else "cpu")),episode_number)
    ...
    if print_things:
        ...
        plt.savefig("-".join([env_name, str('3_1'), "reward_curve_cartpole"]) )
        plt.show()
