...
def discretize(state,action=None):
    disc_state = []
...
for ep in range(episodes+test_episodes):
    ...
    while not done:
        ...
        discrete_state = discretize(state,action=None)
        ...
        if not test:
            ...
            dis_with_action=discretize(state,action)
            Q_curr=q_grid[dis_with_action]
            new_opt_act=np.max(q_grid[discretize(new_state,action)])
            q_grid[dis_with_action]=Q_curr+alpha*(reward+(gamma*new_opt_act)-Q_curr)
